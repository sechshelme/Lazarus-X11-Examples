...
const
  ...
  begin
    ...
    column := gtk_tree_view_column_new_with_attributes('List Items', pixbufrender, 'pixbuf', COL_PIXBUF, nil);
    gtk_tree_view_append_column(GTK_TREE_VIEW(list), column);

    store := gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_INT, G_TYPE_FLOAT, G_TYPE_BOOLEAN, GDK_TYPE_PIXBUF);
  ...
  begin
    ...
    b := boolean(random(2));
    f := Random;
    gtk_list_store_set(store, @iter, LIST_ITEM, str, LIST_NO2, 123, LIST_NO3, f, LIST_TOGGLE, b, COL_PIXBUF, p, -1);
