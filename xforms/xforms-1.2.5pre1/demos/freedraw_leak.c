...
change_color( FL_OBJECT * ob,
			  long        which )
{
    cur_fig->c[ which ] = 255 * fl_get_slider_value( ob );
    fl_mapcolor( FL_FREE_COL1,
				 cur_fig->c[ 0 ], cur_fig->c[ 1 ], cur_fig->c[ 2 ] );
...
create_form_drawfree( void )
{
	...
	fdui->wsli = obj = fl_add_valslider( FL_HOR_SLIDER, 15, 95, 120, 25,
										 "Width" );
    fl_set_object_lalign( obj, FL_ALIGN_TOP );
	...
	fdui->drobj[ 0 ] = obj = fl_add_button( FL_RADIO_BUTTON, 10, 390, 40, 40,
											"@#circle" );
    fl_set_object_lcolor( obj, FL_YELLOW );
    fl_set_object_callback( obj, switch_object, 0 );

	fdui->drobj[ 1 ] = obj = fl_add_button( FL_RADIO_BUTTON, 50, 390, 40, 40,
											"@#square" );
    fl_set_object_lcolor( obj, FL_YELLOW );
    fl_set_object_callback( obj, switch_object, 1 );

	fdui->drobj[ 2 ] = obj = fl_add_button( FL_RADIO_BUTTON, 90, 390, 40, 40,
											"@#8>" );
    fl_set_object_lcolor( obj, FL_YELLOW );
