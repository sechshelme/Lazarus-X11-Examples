...
uses
  ...
  var
    window, vbox, label1, button1: PGtkWidget;
  begin
    gtk_init(@argc, @argv);
    ...
    g_signal_connect(label1, 'clicked', G_CALLBACK(@gtk_main_quit), nil);

    button1:=gtk_button_new_with_label('Programm beenden');
    gtk_box_pack_start(GTK_BOX(vbox),button1 , False, False, 0);
    g_signal_connect(button1, 'clicked', G_CALLBACK(@gtk_main_quit), nil);
