...
const
  ...
  begin
    ...
    WriteLn('b2old: ', Char2BLen);

    Char2BLen := utf8toXChar2b(str2b, Hello);
    WriteLn('b2new: ', Char2BLen);
    ...
    Getmem(pwc, Length(Hello) * 2);
    StringToWideChar(Hello, pwc, Char2BLen);
    ...
    //    Getmem(pwc, Length(Hello) * 2);
    StringToUnicodeChar(Hello, pwc, Char2BLen);
    WriteLn('WideChar: ', Char2BLen);
    for i := 0 to Char2BLen - 1 do begin
      Write(word(pwc[i]), '-');
      //      Write(pwc[i].byte2, ' ');
    ...
    XTextExtents16(fontStructure, pointer(pwc), Char2BLen, @direction, @ascent, @descent, @overall);
  ...
  begin
    while not quit do begin
      ...
      case Event._type of
        ...
        ButtonPress: begin
          if event.xbutton.button = 3 then  begin
            quit := True;
          end;
        end;
      end;
