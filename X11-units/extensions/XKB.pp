...
{
  Automatically converted by H2Pas 1.0.0 from /usr/include/X11/extensions/XKB.h
  The following command line parameters were used:
    /usr/include/X11/extensions/XKB.h
    -s
    -S
    -o
    /n4800/DATEN/Programmierung/mit_GIT/Lazarus/Tutorial/X11/X11-units/extensions/XKB.pp
}
...
function XkbSA_ValOp(a : longint) : a;

function XkbSA_ValScale(a : longint) : a;
...
in define line 561 *)
    ...
    function XkbShiftLevel(n : longint) : n;    
...
implementation
    ...
    function XkbExplicitXIDevice(c : longint) : longint;
    begin
      XkbExplicitXIDevice:=(c(@( not ($ff))))=0;
    end;

  function XkbExplicitXIClass(c : longint) : longint;
  begin
    XkbExplicitXIClass:=(c(@( not ($ff))))=0;
  end;

  function XkbExplicitXIId(c : longint) : longint;
  begin
    XkbExplicitXIId:=(c(@( not ($ff))))=0;
  end;

    function XkbBuildCoreState(m,g : longint) : longint;
    begin
      XkbBuildCoreState:=((g(@($3))) shl 13) or (m(@($ff)));
    end;
...
function XkbSA_ValOp(a : longint) : a;
begin
  XkbSA_ValOp:=a(@(XkbSA_ValOpMask));
end;

function XkbSA_ValScale(a : longint) : a;
begin
  XkbSA_ValScale:=a(@(XkbSA_ValScaleMask));
end;
