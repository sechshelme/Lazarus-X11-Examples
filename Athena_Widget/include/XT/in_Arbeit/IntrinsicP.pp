...
{#ifndef XT_REVISION }
  XT_REVISION = 6;  
{#endif }
  XtVersion = (11*1000)+6;  {#define XtVersion (XT_VERSION * 1000 + XT_REVISION) }
...
{ argument types are unknown }
{ return type might be wrong }   
//function XtCheckSubclass(w,widget_class,message : longint) : longint;

{#endif }
{ _XFUNCPROTOBEGIN }
{ internal; implementation-dependent  }
{ object  }
function _XtWindowedAncestor(para1:TWidget):TWidget;cdecl;external;
{#if (defined(_WIN32) || defined(__CYGWIN__)) && !defined(LIBXT_COMPILATION) }
{__declspec(dllimport) }
{#else }
{#endif }
procedure _XtInherit;cdecl;external;
{ widget  }{ client_data  }{ event  }{ cont  }procedure _XtHandleFocus(para1:TWidget; para2:TXtPointer; para3:PXEvent; para4:PBoolean);cdecl;external;
