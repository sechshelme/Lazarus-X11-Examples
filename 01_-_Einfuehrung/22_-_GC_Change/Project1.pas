...
procedure ChangeGC(var gc:TGC);
var
  values: TXGCValues;
begin
  XGetGCValues(dis,gc,1,@values);
  WriteLn(values.line_width);
  values.line_width:=40;
  XChangeGC(dis,gc,GCForeground or GCBackground,@values);
end;
...
begin
  ...
  Subgc1 := XCreateGC(dis, Subwin1, 0, nil);
  Subgc2 := XCreateGC(dis, Subwin2, 0, nil);
